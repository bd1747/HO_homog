...
def process_gmsh_log(gmsh_log:list, detect_error=True):
    ...
    gmsh_logger.info("**********")
    gmsh_logger.info(f"{len(gmsh_log)} logging messages got from Gmsh : {len(err_msg)} errors, {len(warn_msg)} warnings.")
    if err_msg:
...
panto_geo = mesh_generate_2D.Gmsh2DRVE.pantograph(
                a, b, k, 0.1,
                nb_cells=(10,1),
                soft_mat=True, name='panto_with_soft')
...
panto_part = part.Fenics2DRVE.file_2_FenicsPart(
                panto_geo.mesh_abs_path, material_dict, 
                global_dimensions= panto_geo.gen_vect, subdomains_import=True)
...
boundary_conditions = [
    ...
    {'type': 'Dirichlet', 'constraint': (0.,0.), 'facet_function':boundary_markers, 
        'facet_idx': 11},
    ('Dirichlet', (0.,0.), boundary_markers, 99)
]

#* Step 5 : Defining the load
indicator_fctn = fe.Expression(
    'x[0] >= (LX-Lx_cell)/2.0 && x[0] <= (LX+Lx_cell)/2.0 ? 1 : 0',
    degree=0, LX=LX, Lx_cell=4.*a)
load_area = fe.assemble(indicator_fctn*fe.dx(panto_part.mesh))
load_magnitude = 1.
s_load = load_magnitude / load_area
s_load = fe.Constant((0., s_load)) #forme utilisée dans tuto FEniCS linear elasticity
