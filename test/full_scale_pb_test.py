...
import logging
import dolfin as fe
import ho_homog.full_scale_pb as homog_full
...
def test_reconstruction_vectors():
    ...
    localization_tensors = {
        'U': {
            'u':[
                fe.interpolate(fe.Constant((1.,0.)), displ_fspace).split(),
                fe.interpolate(fe.Constant((0.,1.)), displ_fspace).split(),
            ],
        },
        'E': {
            'u': [
    ...
    exact_sol = {
        'u': fe.project(fe.Expression(
        ("0.5*x[0] + 0.5*cos(x[0])", "0.5*sin(x[1])"),
        degree=2), displ_fspace),
        'eps': fe.project(fe.Expression(
        (" 0.5 - 0.5*sin(x[0])", "0.5*cos(x[1])", "0"),
    ...
    for f_name in reconstr_sol.keys():
        ...
        exact_norm = fe.errornorm(exact_sol[f_name], fe.Constant([0.0]*dim),
                                'L2', degree_rise=0, mesh=mesh)
        difference = fe.errornorm(reconstr_sol[f_name], exact_sol[f_name],'L2',
                                degree_rise=0, mesh=mesh)
        ...
        assert error == approx(0., abs=1e-13)
...
def test_reconstruction_with_constraint():
    ...
    mesh = fe.RectangleMesh(
        fe.Point(-L_x/2, -L_y/2), fe.Point(L_x/2, L_y/2),
        nb_x, nb_y
        )
    ...
    class PeriodicDomain(fe.SubDomain):
        ...
        def map(self, x, y):
            SlaveT = fe.near(x[1], L_y/2., self.tol)
            if SlaveT:
    ...
    localization_tensors = {
        'U': {
            'u':[
                [fe.interpolate(fe.Constant(1.), scalar_fspace)]
            ],
        },
        'E': {
            'u': [
    ...
    exact_sol = fe.project(fe.Expression(
            "0.5*x[0] + x[1] + 0.5*x[1] + 1.*x[0]",
            degree=2), scalar_fspace)

    reconstr_sol = homog_full.reconstruction(
        localization_tensors, macro_fields, function_spaces,
        localization_rules=localization_rules, output_request=('u',))
    ...
    with fe.XDMFFile(str(results_file_path)) as results_file:
        ...
        for field, name, descrpt in data:
            field.rename(name, descrpt)
            results_file.write(field, 0.)
        exact_norm = fe.errornorm(exact_sol, fe.Constant(0.0),
            'L2', degree_rise=0, mesh=mesh)
        difference = fe.errornorm(reconstr_sol, exact_sol,'L2', 0, mesh)
        error = difference/exact_norm
        logger.debug(f'Relative error = {error}') #* ref: 3.2625e-15
        assert error == approx(0., abs=1e-14)
